...
function App() {
  ...
  const [providersOpen, setProvidersOpen] = useStateA(false);
  ...
  function TopRight() {
    ...
    const providersBtn = <button className="iconbtn" title="模型能力 / API" onClick={() => setProvidersOpen(true)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 6h9M17 6h3M4 12h3M11 12h9M4 18h7M15 18h5" /><circle cx="15" cy="6" r="2" /><circle cx="9" cy="12" r="2" /><circle cx="13" cy="18" r="2" /></svg></button>;
    ...
    if (live) return (
      <div className="tb-right" style={{ position: 'relative' }}>{skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}
        {finished && runId ? <>
    ...
    if (screen === 'input') return <div className="tb-right">{skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}</div>;
    if (screen === 'outline') return <div className="tb-right">{skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}<button className="btn primary" onClick={() => setScreen('template')}>确认大纲 · 选模板 {arrow}</button></div>;
    if (screen === 'template') return <div className="tb-right">{skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}<button className="btn" onClick={() => setScreen('outline')}>{back} 返回大纲</button><button className="btn primary" onClick={() => setScreen('output')}>用「{tplName}」· 下一步 {arrow}</button></div>;
    if (screen === 'output') return <div className="tb-right">{skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}<button className="btn" onClick={() => setScreen('template')}>{back} 返回模板</button><button className="btn primary" onClick={() => startRender(mode)}>用 {mode === 'html' ? 'HTML' : 'AI 配图'} 生成 · 开始 {arrow}</button></div>;
    // workbench
    if (!wbDone) return <div className="tb-right">{skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}<button className="btn dis">导出 · 待完成</button></div>;
    return (
      <div className="tb-right" style={{ position: 'relative' }}>
        {skillsBtn}{styleLibBtn}{skillBtn}{providersBtn}{darkBtn}
  ...
  return (
    <div className="app" data-mode={dark ? 'dark' : undefined}>
      ...
      {providersOpen && <Providers onClose={() => setProvidersOpen(false)} />}
    </div>
  );
}
